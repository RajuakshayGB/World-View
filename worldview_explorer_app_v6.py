...
# ==============================================================================
# 0. LOGO PARSING & CATEGORY-BASED THEME DEFINITIONS
# ==============================================================================
def get_base64_logo():
    import base64
    import os
    logo_paths = [
        "app_logo.png",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "app_logo.png") if "__file__" in locals() else "",
        "/workspace/artifacts/app_logo.png",
        "/workspace/knowledge/app_logo.png"
    ]
...
SECTION_THEMES = {
    1: {"bg": "#F8FAFC", "accent": "#0EA5E9", "bg_tint": "#F0F9FF", "border": "#BAE6FD"}, # Slate Blue (Reality)
    2: {"bg": "#FAF5FF", "accent": "#A855F7", "bg_tint": "#F3E8FF", "border": "#E9D5FF"}, # Lilac (Consciousness)
    3: {"bg": "#F0FDFA", "accent": "#14B8A6", "bg_tint": "#CCFBF1", "border": "#99F6E4"}, # Teal (Epistemology)
    4: {"bg": "#ECFDF5", "accent": "#10B981", "bg_tint": "#D1FAE5", "border": "#A7F3D0"}, # Emerald (Truth)
    5: {"bg": "#FFFBEB", "accent": "#D97706", "bg_tint": "#FEF3C7", "border": "#FDE68A"}, # Amber (Religion)
    6: {"bg": "#FAF5FF", "accent": "#7C3AED", "bg_tint": "#F5F3FF", "border": "#DDD6FE"}, # Dark Purple (Death)
    7: {"bg": "#FFF1F2", "accent": "#F43F5E", "bg_tint": "#FFE4E6", "border": "#FECDD3"}, # Rose (Meaning)
    8: {"bg": "#EFF6FF", "accent": "#3B82F6", "bg_tint": "#DBEAFE", "border": "#BFDBFE"}, # Blue (Ethics)
    9: {"bg": "#F0F9FF", "accent": "#06B6D4", "bg_tint": "#E0F2FE", "border": "#BAE6FD"}, # Sky (Practical Virtue)
    10: {"bg": "#FDF6F0", "accent": "#EA580C", "bg_tint": "#FFEDD5", "border": "#FED7AA"}, # Terracotta (Human Nature)
    11: {"bg": "#F5F3FF", "accent": "#8B5CF6", "bg_tint": "#EDE9FE", "border": "#DDD6FE"}, # Indigo (Identity)
    12: {"bg": "#F0FDF4", "accent": "#16A34A", "bg_tint": "#DCFCE7", "border": "#BBF7D0"}, # Forest Green (Free Will)
    13: {"bg": "#F4F7F5", "accent": "#16A34A", "bg_tint": "#E8F5E9", "border": "#C8E6C9"}, # Sage (Society)
    14: {"bg": "#EFF6FF", "accent": "#1D4ED8", "bg_tint": "#D2E3FC", "border": "#B3D1FC"}, # Royal Blue (Liberty)
    15: {"bg": "#FAF6F0", "accent": "#92400E", "bg_tint": "#F5E6D3", "border": "#E6CCB2"}, # Bronze (Order)
    16: {"bg": "#FFF5F5", "accent": "#E11D48", "bg_tint": "#FFEBEE", "border": "#FFCDD2"}, # Burgundy (Equality)
    17: {"bg": "#F5F8F1", "accent": "#65A30D", "bg_tint": "#EAF2D8", "border": "#D2E5A8"}, # Olive Green (Economics)
    18: {"bg": "#FCF5EE", "accent": "#B45309", "bg_tint": "#F7E7D4", "border": "#EDCEB1"}, # Sienna (Culture)
    19: {"bg": "#FEF2F2", "accent": "#DC2626", "bg_tint": "#FEE2E2", "border": "#FCA5A5"}, # Crimson (Political Change)
    20: {"bg": "#F0F4F8", "accent": "#1E3A8A", "bg_tint": "#D9E2EC", "border": "#BCCCDC"}, # Navy Blue (Global Scope)
    21: {"bg": "#F0FDF4", "accent": "#22C55E", "bg_tint": "#DCFCE7", "border": "#BBF7D0"}, # Pure Green (Ecology)
    22: {"bg": "#F0FDF4", "accent": "#10B981", "bg_tint": "#DCFCE7", "border": "#A7F3D0"}, # Mint Green (Animal Ethics)
    23: {"bg": "#F5F3FF", "accent": "#7C3AED", "bg_tint": "#EDE9FE", "border": "#DDD6FE"}, # Electric Violet (Technology)
    24: {"bg": "#ECFEFF", "accent": "#06B6D4", "bg_tint": "#CFFAFE", "border": "#A5F3FC"}, # Turquoise (Future)
    25: {"bg": "#FFF7ED", "accent": "#F97316", "bg_tint": "#FFEDD5", "border": "#FED7AA"}  # Peach (Pluralism)
}
...
<style>
    ...
    /* Premium Colorful Language Selectbox / Toggle */
    div[data-testid="stSelectbox"] {
        border: 2px solid transparent !important;
        background-image: linear-gradient(rgba(255,255,255,1), rgba(255,255,255,1)), 
                          linear-gradient(135deg, #6366F1, #D946EF, #3B82F6) !important;
    ...
    /* Smooth global transitions for background tinting */
    .stApp {
...
def load_and_parse_docx(uploaded_file=None):
    if uploaded_file is not None:
        ...
        doc = None
        import os
        base_filename = "World_View_100_Questions.docx"
        script_dir = os.path.dirname(os.path.abspath(__file__)) if "__file__" in locals() else ""
        
        fallback_paths = [
            base_filename,
            os.path.join(script_dir, base_filename) if script_dir else "",
            "World_View_100_Questions.docx",
            os.path.join(script_dir, "World_View_100_Questions.docx") if script_dir else "",
            ...
            "/workspace/artifacts/World_View_100_Questions.docx",
            "/workspace/knowledge/World_View_100_Questions.docx"
        ]
...
with header_col1:
    ...
    if base64_logo:
        logo_html = f"""
        <a href="/" target="_self" style="text-decoration: none; display: flex; align-items: center; gap: 12px; margin-top: 5px; margin-bottom: 5px;">
            <img src="data:image/png;base64,{base64_logo}" width="42" height="42" style="border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1.0)'">
        ...
        logo_html = """
        <a href="/" target="_self" style="text-decoration: none; display: flex; align-items: center; gap: 12px; margin-top: 5px; margin-bottom: 5px;">
            <span style='font-size: 2.2rem; transition: transform 0.2s;' onmouseover="this.style.transform='scale(1.15)'" onmouseout="this.style.transform='scale(1.0)'">🧭</span>
            <span style='font-family: "Cinzel", serif; font-weight: 700; font-size: 1.45rem; color: #0F172A; letter-spacing: 0.05em;'>WORLDVIEW COMPASS</span>
        </a>
        """
...
if questions is None or len(questions) == 0:
    st.error("❌ Critical Error: Could not locate the bilingual document `World_View_100_Questions.docx`. Please upload it in the sidebar!")
    st.stop()
...
if not st.session_state.started and not st.session_state.completed:
    # Gentle landing page tint (Section 1 default slate blue)
    st.markdown("""
    <style>
        .stApp {
...
if st.session_state.started and not st.session_state.completed:
    ...
    # Category-based Dynamic Page Tinting
    q_sect = q.get("section_num", 1)
    theme = SECTION_THEMES.get(q_sect, SECTION_THEMES[1])
    st.markdown(f"""
    <style>
        .stApp {{
        ...
        .museum-card {{
            background-color: {theme['bg_tint']} !important;
            border-color: {theme['border']} !important;
        }}
        .category-header {{
            color: {theme['accent']} !important;
        }}
        /* Hover glow for choice cards based on current section */
        .main div[data-testid="stButton"] button:hover {{
            border-color: {theme['accent']} !important;
            background-color: {theme['bg_tint']} !important;
            color: {theme['accent']} !important;
            box-shadow: 0 6px 18px {theme['accent']}22 !important;
            transform: translateY(-2px) !important;
        }}
        /* Primary active selection styling */
        .main div[data-testid="stButton"] button[data-testid="baseButton-primary"] {{
            background-color: #0F172A !important;
            color: #F8FAFC !important;
            border-color: #0F172A !important;
            box-shadow: 0 6px 16px {theme['accent']}33 !important;
        }}
    </style>
    """, unsafe_allow_html=True)
...
# ==============================================================================
    ...
    # Soft background with radiant heading styles
    st.markdown("""
    <style>
        .stApp {
    ...
    <div style='text-align: center; margin-bottom: 40px;'>
        <h1 class='reveal-header-title' style='font-size: 3.4rem;'>🧭 {ui['result_title']}</h1>
