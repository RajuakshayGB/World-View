...
def load_and_parse_docx(uploaded_file=None):
    ...
    if uploaded_file is not None:
        ...
        doc = None
        # Fallback list checking sandbox absolute paths, current directory, and script parent directory
        import os
        base_filename = "the-compass-of-human-perspectives-bilingual-v2.docx"
        script_dir = os.path.dirname(os.path.abspath(__file__)) if "__file__" in locals() else ""
        
        fallback_paths = [
            # 1. Check local repository directory (for deployed Streamlit app)
            base_filename,
            ...
            # 2. Check local sandbox environment paths (for Notebook workspace)
        ...
        # Filter out empty paths and load
